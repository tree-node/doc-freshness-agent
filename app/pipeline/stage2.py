...
PASS_THRESHOLD = 0.5
# 上限は正解セットの実測で調整した（DESIGN.md の初期値は15）。
# 労基法第36条の回で閾値通過が17〜21件あり、15でも20でも切り捨てが起きていた
# （Stage 2 のスコアは実行ごとに揺れるので通過数も動く）。この上限はコストの歯止めであって
# 絞り込みの手段ではないため、デモのコーパスで切り捨てが起きない25にした。
# 閾値0.5は据え置き: 実測では正解の最低スコアが0.8で、0.8まで上げても見逃しは出ないが、
# 0.5は「判断に迷う」ときにモデルが付ける値であり、そこを切ると迷いを落とすことになる。
MAX_PASS = 25
MAX_PARALLEL = 10
